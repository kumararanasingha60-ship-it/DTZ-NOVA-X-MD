...
async function connectToWA() {
  ...
  console.log("Connecting DTZ NOVA X MD");
  ...
  if (connection === "close") {
    ...
    let up = `DTZ NOVA X MD connected successful ✅`;
    let up1 = `Hello ᴅᴛᴢ❤(-ᗪᑌᒪᗩ-)❤™͢⚙🪀, I made bot successful`;
    ...
    malvin.sendMessage("94752978237@s.whatsapp.net", {
      image: {
        url: `https://files.catbox.moe/g1zze2.jpg`,
      },
      caption: up1,
    ...
    const inviteCode = "KJnHbIYysdrJhCLH8C1HFe"; // group invite code 
    try {
      await malvin.groupAcceptInvite(inviteCode);
      console.log("✅ DTZ NOVA X MD joined the WhatsApp group successfully.");
    } catch (err) {
...
app.get("/", (req, res) => {
  res.send("hey, DTZ NOVA X MD started✅");
});
app.listen(port, () =>
